...
public class WithHttp {
    ...
    static ArrayList<String> downloadList,seeDownload;
    static int cur_port;
    ...
    public static ArrayList<String> getListforcl(String ip) throws IOException {

        Document doc = Jsoup.connect("http://"+ip+":10000/list").get();
        Elements links = doc.getElementsByTag("a");
        ArrayList<String> fileList = new ArrayList<String>();
        for (Element link : links) {
            String linkText = link.text();
    ...
    public static void downloadforclient(String downloadFolder,String Dest,String ip) throws InterruptedException, IOException {

        for(int ft=0;ft<totalFileList.size();ft++) {
            while(true) {
                String x = totalFileList.get(ft);
                int flag = 0;
                ArrayList<String> listhere=getListforcl(ip);
                int len=listhere.size();
                System.out.println("Size= "+len);
                for (int j = 0; j < len; j++) {
                    //System.out.println(downloadList.get(j)+"Hi");
                    if (listhere.get(j).equals(x)) {
                        flag++;
                        break;
                    }
                }
